...
class BarcodeGenerator
  ...
  def BarcodeGenerator.get_barcode_image(barcode_string, format=Gbarcode::BARCODE_39)
    return BarcodeGenerator.get_subprocess_output do
      barcode_generator = BarcodeGenerator.new(format)
      $stdout.write(barcode_generator.get_barcode_image(barcode_string))
    end
  end

  def initialize(format = Gbarcode::BARCODE_39)
  ...
  def get_barcode_image(string_to_encode)
    ...
    png_barcode = convert_eps_to_png(eps_barcode)
    return png_barcode
  end
  ...
  def get_barcode_eps(string_to_encode)
    ...
    Gbarcode.barcode_encode(barcode_object, @format)
    return BarcodeGenerator.get_subprocess_output do
